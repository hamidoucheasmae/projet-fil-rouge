$(document).ready(function(){
    ...
    $.ajax({
      ...
      success:function(data) {
        ...
        for (var i = 0; i < documentsList.length; i++) {
          $(".documentList").append('<tr class="text-center"><td class="align-middle">' + documentsList[i]["name"] + '</td><td class="align-middle">' + documentsList[i]["date_expiration"] + '</td><td class="align-middle">' + documentsList[i]["etat"] + '</td><td class="align-middle">' +'</td><td class="align-middle"><span class="btn update-btn" data-toggle="modal" data-target="#exampleModal"><img src="/assets/images/edit.svg" width="40" height="40"></span><span class="btn delete-btn ml-2 mb-0 mt-0"><img src="/assets/images/trash.svg" width="40" height="40"></span><input type="hidden" value="" class="stdID"></td></tr>');
        }
      }
    })
  }
  ...
  $("body").on("click",".delete-btn",function() {
    var index = $(this).parents("tr").index();
    $(".documentList .stdID").val(documentsList[index]["id"]);
    var studentID =$(".documentList .stdID").val();
    $.ajax({
